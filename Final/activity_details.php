<?php
session_start();
// ตั้งค่าโซนเวลาไทยเพื่อให้การเช็ควันที่ปัจจุบันถูกต้อง
...
// 1. ดึงข้อมูลบูธ ชื่อร้าน และวันที่จัดงาน (เพิ่มการ JOIN ตาราง events)
$stmt = $conn->prepare("
...
// 2. ดึงรอบกิจกรรมและจำนวนคนจอง
// แก้ไข Query: เพิ่มเงื่อนไข r.status != 'cancelled' เพื่อให้นับเฉพาะคนที่มีสิทธิ์เข้างานจริงๆ
...
// แก้ไข Query ตรวจสอบประวัติ: ต้องเช็คด้วยว่าประวัติที่มีไม่ใช่รายการที่ยกเลิกไปแล้ว
$booked_ids = [];
if ($is_logged_in) {
    $stmt_check = $conn->prepare("SELECT activity_id FROM user_activity_reservations WHERE user_id = ? AND status != 'cancelled'");
...
<head>
    ...
    <style>
        ...
        .card-booking.finished-card { opacity: 0.8; background-color: #eee; filter: grayscale(1); }
    ...
    <div class="container">
        ...
        <?php if ($is_future_event): ?>
            <div class="alert alert-info py-2 border-0 rounded-4 d-inline-block small mb-0">
                <i class="fas fa-calendar-alt me-1"></i> กิจกรรมนี้จะเปิดในวันที่ <?php echo date('d/m/Y', strtotime($event_date)); ?>
            </div>
    ...
    <div class="row g-3">
        <?php if (count($activities) > 0): ?>
            <?php foreach ($activities as $act): 
                ...
                // --- ส่วนที่เพิ่มใหม่: เช็คว่าเลยเวลาเริ่มรอบหรือยัง ---
                $start_time_ts = strtotime($event_date . ' ' . $act['start_time']); // รวมวันที่งานกับเวลาเริ่มรอบ
                $now_ts = time();
                $is_started = ($now_ts >= $start_time_ts); // ถ้าเวลาปัจจุบัน >= เวลาเริ่มรอบ = เริ่มแล้ว
            ?>
            <div class="col-12 col-sm-6 col-lg-4">
                <div class="card card-booking <?php echo $is_booked ? 'booked' : ''; ?> <?php echo ($status == 'finished' || $status == 'cancelled' || ($is_event_day && $is_started && !$is_booked)) ? 'finished-card' : ''; ?>">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        <?php elseif ($status == 'finished' || ($is_event_day && $is_started)): ?>
                            <span class="badge bg-secondary rounded-pill">เริ่ม/จบกิจกรรมแล้ว</span>
